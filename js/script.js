...
menu_open_btn.addEventListener("click", function (evt) {
	if (!menu_open_btn.classList.contains("main-navigation__btn--open-menu")) {
		...
		setTimeout(function(){
			...
			setTimeout(function(){
				main_content.classList.remove("main-content-down");
				main_content.classList.add("main-content-up");
			}, 300);
			setTimeout(function(){
    			main_navigation__list_item.forEach(n => n.classList.remove("close-menu"));
    			main_content.classList.remove("main-content-up");
			}, 600);
	}
}); 